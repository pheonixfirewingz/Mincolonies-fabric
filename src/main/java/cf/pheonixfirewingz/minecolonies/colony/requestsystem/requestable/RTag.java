 ...
 */
public class RTag implements IDeliverable
{
    /**
     * Set of type tokens belonging to this class.
     */
    private final static Set<TypeToken<?>> TYPE_TOKENS = ReflectionUtils.getSuperClasses(TypeToken.of(RTag.class)).stream().filter(type -> !type.equals(TypeConstants.OBJECT)).collect(Collectors.toSet());
     ...
     */
    public RTag(@NotNull final Tag<Item> tag, final int count)
    {
     ...
     */
    public RTag(@NotNull final Tag<Item> tag, final int count, final int minCount)
    {
     ...
     * @param minCount the min count.
     */
    public RTag(@NotNull final Tag<Item> tag, @NotNull final ItemStack result, final int count, final int minCount)
    {
        this.theTag = tag;
        this.result = result;
        this.count = count;
    ...
    {
        return new RTag(theTag, result, newCount, minCount);
    }
     ...
     * @return the compound.
     */
    public static CompoundTag serialize(final IFactoryController controller, final RTag input)
    {
    ...
    public static void serialize(final IFactoryController controller, final PacketByteBuf buffer, final RTag input)
    {
    ...
    public static RTag deserialize(final IFactoryController controller, final PacketByteBuf buffer)
    {
        final Tag<Item> theTag = ItemTags.getCollection().get(ResourceLocation.tryCreate(buffer.readString(32767)));
        final ItemStack result = buffer.readBoolean() ? buffer.readItemStack() : ItemStack.EMPTY;
        final int count = buffer.readInt();
        final int minCount = buffer.readInt();

        return new RTag(theTag, result, count, minCount);
    }
     ...
     * @return the deliverable.
     */
    public static RTag deserialize(final IFactoryController controller, final CompoundTag compound)
    {
        final Tag<Item> theTag = ItemTags.makeWrapperTag(compound.getString(NBT_TAG));
