package cf.pheonixfirewingz.minecolonies.colony.compat;

import cf.pheonixfirewingz.minecolonies.Items.ModTags;
import cf.pheonixfirewingz.minecolonies.Minecolonies;
import cf.pheonixfirewingz.minecolonies.colony.*;
import cf.pheonixfirewingz.minecolonies.crafting.ItemStorage;
...
import static cf.pheonixfirewingz.minecolonies.utils.ItemStackUtils.*;
import static cf.pheonixfirewingz.minecolonies.utils.constant.Constants.*;
import static cf.pheonixfirewingz.minecolonies.utils.constant.NbtTagConstants.TAG_SAP_LEAF;
...
public class CompatibilityManager implements ICompatibilityManager
{
     ...
     */
    private static final BooleanProperty checkDecay     = BooleanProperty.of("check_decay");
    private static final BooleanProperty decayable      = BooleanProperty.of("decayable");
    public static final  IntProperty DYN_PROP_HYDRO = IntProperty.of("hydro", 1, 4);
    public static final  IntProperty TREE_DISTANCE  = IntProperty.of("distance", 1, 7);
    ...
    {
        final List<ItemStack> stacks = StreamSupport.stream(Spliterators.spliteratorUnknownSize(Registry.ITEM.iterator(), Spliterator.ORDERED), true)
                                           .flatMap(item ->
                                             {
    ...
    public boolean isPlantable(final ItemStack itemStack)
    {
        ...
        {
            if (itemStack.getItem().getTranslationKey().toString().equals(string))
            {
                return true;
            }
            ...
            if (split.length == 2)
            {
                for (final Identifier tag : itemStack.getItem().getTags())
                {
                    if (tag.toString().contains(":" + split[1]) && itemStack.getItem().getTranslationKey().getBytes().equals(split[0]))
                    {
                        return true;
                    }
                }
    ...
    public boolean isOre(@NotNull final ItemStack stack)
    {
        ...
        for (final Identifier tag : stack.getItem().getTags())
        {
    ...
    @Override public void writeNbt(@NotNull CompoundTag compound)
    {
    ...
    @Override public void readNbt(@NotNull CompoundTag compound)
    {
    ...
    public Tuple<ItemStack, Integer> getRandomEnchantmentBook(final int buildingLevel)
    {
        ...
        return new Tuple<>(getEnchantedItemStack(new EnchantmentLevelEntry(Objects.requireNonNull(Registry.ENCHANTMENT.get(new Identifier(ench.getA()))), ench.getB())),
          ench.getB());
    }
    ...
    private void discoverOres()
    {
        ...
        Minecolonies.log("Finished discovering Ores");
    }

    private void discoverSaplings()
    {

        for (final Item item : ItemTags.SAPLINGS.values())
        {
        ...
        Minecolonies.log("Finished discovering saplings");
    }
    ...
    private void discoverCompostableItems()
    {
        ...
        Minecolonies.log("Finished discovering compostables");
    }
    ...
    private void discoverPlantables()
    {
        ...
        Minecolonies.log("Finished discovering plantables");
    }
    ...
    private void discoverFuel()
    {
        if (fuel.isEmpty())
        {
            fuel.addAll(ImmutableList.copyOf(allItems.stream().filter(FurnaceBlockEntity::canUseAsFuel).map(ItemStorage::new).collect(Collectors.toList())));
        }
        Minecolonies.log("Finished discovering fuel");
    }
    ...
    private void discoverFood()
    {
        ...
        Minecolonies.log("Finished discovering food");
    }
    ...
    private void discoverLuckyOres()
    {
        if (luckyOres.isEmpty())
        {
            for (final String ore : MinecoloniesAPIProxy.getInstance().getConfig().getServer().luckyOres.get())
            {
                ...
                if (split.length < 2)
                {
                    Minecolonies.logWarn("Wrong configured ore: " + ore);
                    continue;
                }

                final Item item = Registry.ITEM.get(new Identifier(split[0]));
                if (item == null || item == Items.AIR)
                {
                    Minecolonies.logWarn("Invalid lucky block: " + ore);
                    continue;
                }
                ...
                {
                    Minecolonies.logWarn("Ore has invalid rarity: " + ore);
                }
            }
        }
        Minecolonies.log("Finished discovering lucky oreBlocks");
    }
    ...
    private void discoverRecruitCosts()
    {
        if (recruitmentCostsWeights.isEmpty())
        {
            for (final String itemString : MinecoloniesAPIProxy.getInstance().getConfig().getServer().configListRecruitmentItems.get())
            {
                ...
                if (split.length < 2)
                {
                    Minecolonies.logWarn("Wrong configured recruitment cost: " + itemString);
                    continue;
                }
                ...
                if (item == null || item == Items.AIR)
                {
                    Minecolonies.logWarn("Invalid recruitment item: " + item);
                    continue;
                }
                ...
                {
                    Minecolonies.logWarn("Invalid recruitment weight for: " + item);
                }
            }
        }
        Minecolonies.log("Finished discovering recruitment costs");
    }
    ...
    private void discoverDiseases()
    {
        if (diseases.isEmpty())
        {
            for (final String disease : MinecoloniesAPIProxy.getInstance().getConfig().getServer().diseases.get())
            {
                ...
                if (split.length < 3)
                {
                    Minecolonies.logWarn("Wrongly configured disease: " + disease);
                    continue;
                }
                ...
                try
                {
                    ...
                    for (int i = 2; i < split.length; i++)
                    {
                        ...
                        final Item item = Registry.ITEM.get(new Identifier(theItem[0], theItem[1]));
                        if (item == null || item == Items.AIR)
                        {
                            Minecolonies.logWarn("Invalid cure item: " + disease);
                            continue;
                        }
        ...
        Minecolonies.log("Finished discovering diseases");
    }
    ...
    private void discoverSifting()
    {
        for (final String string : MinecoloniesAPIProxy.getInstance().getConfig().getServer().sifterMeshes.get())
        {
            ...
            if (mesh.length != 2)
            {
                Minecolonies.logWarn("Couldn't parse the mesh: " + string);
                continue;
            }
            ...
            try
            {
                ...
                final Item theItem = Registry.ITEM.get(new Identifier(item[0], item[1]));

                if (theItem == null)
                {
                    Minecolonies.logWarn("Couldn't find item for mesh: " + string);
                    continue;
                }
            ...
            {
                Minecolonies.logWarn("Couldn't retrieve probability for mesh: " + string);
                Minecolonies.logWarn(ex.getMessage());
            }
        }

        Minecolonies.log("Finished initiating sifter config");
    }
    ...
    private void discoverEnchantments()
    {
        for (final String string : MinecoloniesAPIProxy.getInstance().getConfig().getServer().enchantments.get())
        {
            ...
            if (split.length != 4)
            {
                Minecolonies.logWarn("Invalid enchantment mode setting: " + string);
                continue;
            }
            ...
            try
            {
                ...
                if (Registry.ENCHANTMENT.get(new Identifier(enchantment)) == null)
                {
                    Minecolonies.logWarn("Enchantment: " + enchantment + " doesn't exist!");
                    continue;
                }
            ...
            {
                Minecolonies.logWarn("Invalid integer at pos 1, 3 or 4");
            }
        }
    }
    ...
    private void discoverCrusherModes()
    {
        for (final String string : MinecoloniesAPIProxy.getInstance().getConfig().getServer().crusherProduction.get())
        {
            ...
            if (split.length != 2)
            {
                Minecolonies.logWarn("Invalid crusher mode setting: " + string);
                continue;
            }
            ...
            final Item item1 = Registry.ITEM.get(new Identifier(firstItem[0], firstItem[1]));
            final Item item2 = Registry.ITEM.get(new Identifier(secondItem[0], secondItem[1]));
            ...
            {
                Minecolonies.logWarn("Error getting metaData");
                Minecolonies.logWarn(ex.getMessage());
            }
        }
    }
    ...
    {
        final CompoundTag compound = NBTUtil.writeBlockState(state);
        storage.getItemStack().writeNbt(compound);
        return compound;
    }

    private static Tuple<BlockState, ItemStorage> readLeafSaplingEntryFromNBT(final CompoundTag compound)
    {
        return new Tuple<>(NBTUtil.readBlockState(compound), new ItemStorage(ItemStack.fromNbt(compound), false, true));
    }
    ...
    private void discoverFreeBlocksAndPos()
    {
        for (final String s : MinecoloniesAPIProxy.getInstance().getConfig().getServer().freeToInteractBlocks.get())
        {
            try
            {
                final Block block = Registry.BLOCK.get(new Identifier(s));
