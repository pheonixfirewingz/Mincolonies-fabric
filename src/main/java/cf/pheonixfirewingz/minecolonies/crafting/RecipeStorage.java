package cf.pheonixfirewingz.minecolonies.crafting;

import cf.pheonixfirewingz.minecolonies.MinecoloniesItems;
import cf.pheonixfirewingz.minecolonies.colony.MinecoloniesAPIProxy;
...
public class RecipeStorage implements IRecipeStorage
{
     ...
     */
    public RecipeStorage(final IToken<?> token, final List<ItemStack> input, final int gridSize, @NotNull final ItemStack primaryOutput, final Block intermediate, final Identifier source, final Identifier type, final List<ItemStack> altOutputs, final List<ItemStack> secOutputs, final Identifier lootTable)
    {
        ...
        this.secondaryOutputs = secOutputs != null ? secOutputs.stream().filter(i -> i.getItem() != MinecoloniesItems.builders_wand).collect(Collectors.toList()): this.calculateSecondaryOutputs();
    ...
    private List<ImmutableItemStorage> calculateCleanedInput()
    {
        ...
        {
            if (ItemStackUtils.isEmpty(stack) || stack.getItem() == MinecoloniesItems.builders_wand)
            {
                continue;
            }
    ...
    private List<ItemStack> calculateSecondaryOutputs()
    {
        ...
        for (final ItemStack stack : input)
        {
            if (stack.getItem() == MinecoloniesItems.builders_wand)
            {
                continue;
            }
